# GUI tool for watching data over serial
# Author: Patrick Brennan (AM2i9)

# requires pyserial and dearpygui packages
...
dpg.create_context()
dpg.create_viewport(title='Big Brother', width=1340, height=1000, resizable=False, clear_color=(25,25,25))
...
# Display log messsage that is not a value
def _log_msg(msg: str):
    ...
    if dpg.get_value(message_log_scroll):
        dpg.set_y_scroll(message_log_window, -1)
...
def _log_var(var_name: str, value_str: str):
    try:
        ...
        # we're just gonna ignore it because, as Chester Benning once said, "In the end, it doesn't even matter"
        pass
...
with dpg.window(label="Message Log", width=450, height=300, pos=(10,130)) as serial_win:
    ...
    with dpg.group(horizontal=True):
        ...
        message_log_scroll = dpg.add_checkbox(label="Scroll", default_value=True)
